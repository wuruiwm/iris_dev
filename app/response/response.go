...
//通用失败返回
func Error(c iris.Context,msg string){
	Return(c,ErrorCode,msg,nil)
}

//通用返回
func Return(c iris.Context,code int,msg string,data interface{}){
	r := &Response{
		Code:code,
		Msg:msg,
		Data:data,
...
//通用json返回
func JSON(c iris.Context,r *Response){
	_, _ = c.JSON(r)
}

//控制器内捕获错误 并以通用失败返回
func RecoverError(c iris.Context,defaultMsg string){
